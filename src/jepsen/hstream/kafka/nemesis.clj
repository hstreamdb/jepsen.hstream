(ns jepsen.hstream.kafka.nemesis
  ...
  (:require [clojure [set :as set]]
            ...
            [jepsen.hstream.kafka.db.redpanda :as rdb]
...
(defn package
  ...
  (let [faults (set (:faults opts [:partition :kill :pause :clock]))
    ...
    (nc/compose-packages [(nc/partition-package opts)
                          (nc/db-package opts)
                          (membership-package opts)])))
;; Note: Docker does not support clock-related nemesis.
;;       So we just expanded `nc/nemesis-packages` and
;;       omitted `nc/clock-package`.
;; TODO: Jepsen 0.3.x added some new nemesis such as
;;       packet and file-corruption. Add them if needed.
;; (-> opts
;;     nc/nemesis-packages
