(ns jepsen.hstream.client
  ...
  (:import [io.hstream HStreamClient HStreamClientBuilder ProducerBuilder HRecord HRecordBuilder
                       ...
                       HRecordReceiver ReceivedHRecord Responder Stream]
...
(defn delete-all-streams [client]
  (let [all-streams (list-streams client)]
    (dorun (map #(delete-stream client %) all-streams))))

(defn create-producer [client stream-name]
  (.build
   (ProducerBuilder/.stream
    (HStreamClient/.newProducer client) stream-name)))

(defn write-data [producer data-to-write]
...
(defn subscribe [client sub-id stream offset timeout]
  "offset can be one of :latest :earliest and {:batch-id n1 :batch-index n2}"
  (let [real-offset (case offset
                      :earliest (SubscriptionOffset. (SubscriptionOffset$SpecialOffset/EARLIEST))
                      :latest   (SubscriptionOffset. (SubscriptionOffset$SpecialOffset/LATEST))
                      :else (RecordId. (:batch-id offset) (:batch-index offset)))
        subscription (.build
                      (.ackTimeoutSeconds
                       (.offset
                        (.stream
...
(defn unsubscribe [client sub-id]
  (.deleteSubscription client sub-id))

(defn consume [client sub-id callback]
  (let [hrecord-receiver (reify HRecordReceiver
